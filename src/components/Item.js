  ...
  return (
    <div className='mb-2'>
      <div className='relative mb-8 imageHolder'>
        ...
        {!showAddItem && <AddItem onMouseEnter={displayAddItems}/>}
...
function AddItem({ onMouseEnter }) {
  return (
    <div onMouseEnter={onMouseEnter} className="transition-opacity absolute -bottom-6 left-1/2 transform -translate-x-1/2 flex justify-between items-center w-max h-4 md:h-8 bg-white p-6 rounded-full border border-gray-400 gap-4 cursor-pointer">
...
function ChangeQuantity({ quantity, onIncrement, onDecrement, onMouseLeave }) {
  return (
    <div onMouseLeave={onMouseLeave} className="transition-opacity absolute z-10 -bottom-6 left-1/2 transform -translate-x-1/2 flex justify-between items-center w-max h-4 md:h-8 p-6 rounded-full border border-red-800 gap-8 bg-color-orange">
